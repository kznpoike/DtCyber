...
static FILE *dd8xxMount(char *deviceName, DiskParam *dp)
    ...
    */
    ...
    /*
    **  Try to open existing disk image. If we are successful do a size
    **  check. If the file is not of the expected size close if and NULL 
    **  out the stream handle so that we fall into the create logic which
    **  will truncate and rebuild it.
    */
    fcb = fopen(fname, "r+b");
    if (fcb != NULL)
        {
        struct stat sb;
        if (fstat(fileno(fcb), &sb) != 0)
            {
            fclose(fcb);
            fcb = NULL;
            }
    ...
    if (fcb == NULL)
        ...
        */
        ...
        if (dp->baseAddr == MAP_FAILED)
            {
            sprintf(msg, "(dd8xx  ) Failed to map %s with error code %d\n",
                fname, errno);
            opDisplay(msg);
            dp->baseAddr = NULL;
            dp->maxOffset = 0;
...
static PpWord dd8xxReadClassic(DiskParam *dp, FILE *fcb)
    ...
    */
    if (dp->bufPtr == NULL)
        {
        dp->bufPtr = memcpy(dp->buffer, &dp->baseAddr[dp->currPos], dp->sectorSize);
    ...
    */
    ...
    return (*dp->bufPtr++);
/*
    PpWord data = dp->baseAddr[dp->currPos];
    dp->currPos += sizeof(PpWord);
    return data;
*/
    }
    ...
    */
    if (dp->bufPtr == dp->buffer + SectorSize)
        ...
        if (dp->flushCount > 20)
            ...
            */
            msync(dp->baseAddr, dp->maxOffset + 1, MS_ASYNC);
            dp->flushCount = 0;
            }
        }
    }
...
static PpWord dd8xxReadPacked(DiskParam *dp, FILE *fcb)
    ...
    */
    if (dp->bufPtr == NULL)
        {
        dp->bufPtr = dp->buffer;
        memcpy(sector, &dp->baseAddr[dp->currPos], dp->sectorSize);
        dp->currPos += dp->sectorSize;
...
static void dd8xxWritePacked(DiskParam *dp, FILE *fcb, PpWord data)
    ...
    */
    if (dp->bufPtr == dp->buffer + SectorSize)
        ...
        **  Write the sector.
        */
        memcpy(&dp->baseAddr[dp->currPos], sector, dp->sectorSize);
        dp->currPos += dp->sectorSize;
        }
    }
...
static void dd844SetClearFlaw(DiskParam *dp, PpWord flawState)
    ...
    */
    ...
    dp->sector   = 2;
    dp->currPos = dd8xxSeek(dp);
    dd8xxSectorRead(dp, fcb, mySector);
...
static char *dd8xxFunc2String(PpWord funcCode)
    ...
    case Fc8xxOnSectorStatus:
        return "OnSectorStatus";

    case Fc8xxDriveRelease:
        return "DriveRelease";

    case Fc8xxReturnCylAddr:
        return "ReturnCylAddr";
    ...
    case Fc8xxGapWriteVerify:
        return "GapWriteVerify";

    case Fc8xxGapReadCheckword:
        return "GapReadCheckword";
