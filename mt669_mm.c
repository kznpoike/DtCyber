...
void mt669Init(u8 eqNo, u8 unitNo, u8 channelNo, char *deviceName)
    ...
    */
    if (deviceName != NULL)
        ...
        */
        tp->baseAddr = mmap(NULL, tp->maxOffset + 1, PROT_READ,
...
void mt669LoadTape(char *params)
    ...
    */
    if (unitMode == 'w')
        ...
        if (fcb != NULL)
            ...
            */
            tp->maxOffset = MaxTapeSize - 1;
            char dummy[] = "1234";
            fseek(fcb, tp->maxOffset-strlen(dummy), SEEK_SET);
    ...
    ** Set up the memory mapping for the device
    */
    tp->baseAddr = mmap(NULL, tp->maxOffset + 1, mapProt, MAP_SHARED,
        fileno(fcb), 0);
    if (tp->baseAddr == MAP_FAILED)
        {
